import type { SsrFPolicy } from "./network-security.js";
...
function deriveDefaultBrowserCdpPortRange(browserControlPort: number): { start: number; end: number } {
  ...
  const end = Math.min(
    65535,
    start + (DEFAULT_BROWSER_CDP_PORT_RANGE_END - CDP_PORT_RANGE_START),
  );
...
export function resolveBrowserConfig(
  ...
  rootConfig?: StandaloneRuntimeConfig,
): ResolvedBrowserConfig {
  const enabled = cfg?.enabled ?? DEFAULT_OPENCLAW_BROWSER_ENABLED;
  const evaluateEnabled = cfg?.evaluateEnabled ?? DEFAULT_BROWSER_EVALUATE_ENABLED;
  const controlPort =
    cfg?.controlPort ??
    rootConfig?.browser?.controlPort ??
