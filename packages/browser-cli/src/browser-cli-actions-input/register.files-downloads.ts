...
async function normalizeUploadPaths(paths: string[]): Promise<string[]> {
  return paths.map((candidate) => {
    ...
    if (!fs.existsSync(resolved)) {
      throw new Error(`upload path does not exist: ${candidate}`);
    }
    return resolved;
  });
}
...
export function registerBrowserFilesAndDownloadsCommands(
  ...
  browser
    ...
    .requiredOption("--paths <paths...>", "File paths to upload")
    ...
    .action(async (opts, cmd) => {
      const { parent, profile } = resolveBrowserActionContext(cmd, parentOpts);
      const requestedPaths = Array.isArray(opts.paths) ? opts.paths.map(String) : [];
      const normalizedPaths = await normalizeUploadPaths(requestedPaths);
      const { timeoutMs, targetId } = resolveTimeoutAndTarget(opts);
      await runBrowserPostAction({
        ...
        timeoutMs: timeoutMs ?? 20000,
        describeSuccess: () => `upload armed for ${requestedPaths.length} file(s)`,
      });
    });
  ...
  browser
    ...
    .argument("[path]", "Save path for the downloaded file")
    .option("--target-id <id>", "CDP target id (or unique prefix)")
    .option(
      "--timeout-ms <ms>",
  ...
  browser
    ...
    .option("--prompt-text <text>", "Prompt response text")
    .option("--target-id <id>", "CDP target id (or unique prefix)")
    .option(
      "--timeout-ms <ms>",
    ...
    .action(async (opts, cmd) => {
      ...
      await runBrowserPostAction({
        ...
        body: {
          accept,
          promptText: opts.promptText?.trim() || undefined,
          targetId,
          timeoutMs,
        },
        timeoutMs: timeoutMs ?? 20000,
