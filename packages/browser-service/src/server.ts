...
import { loadRuntimeConfig, resolveBrowserConfig } from "./config.js";
...
export async function startBrowserControlServerFromConfig(): Promise<BrowserServerState | null> {
  ...
  try {
    ...
    if (ensured.generatedToken) {
      logServer.info("No browser auth configured; generated auth.token automatically.");
    }
  } catch (err) {
  ...
  const server = await new Promise<Server>((resolve, reject) => {
    ...
    logServer.warn(`browser server failed to bind 127.0.0.1:${port}: ${String(err)}`);
    return null;
  });
