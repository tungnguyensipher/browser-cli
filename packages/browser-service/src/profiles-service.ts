...
import { isLoopbackHost, type BrowserProfileConfig, type StandaloneRuntimeConfig } from "@aibrowser/browser-shared";
import { resolveOpenClawUserDataDir } from "./chrome.js";
import { deriveDefaultBrowserCdpPortRange, parseHttpUrl, resolveProfile } from "./config.js";
...
import { loadStandaloneConfigFile, writeStandaloneConfigFile } from "./runtime-config-store.js";
...
export function createBrowserProfilesService(ctx: BrowserRouteContext) {
  ...
  const createProfile = async (params: CreateProfileParams): Promise<CreateProfileResult> => {
    ...
    const cfg = loadStandaloneConfigFile();
    ...
    await writeStandaloneConfigFile(nextConfig);
